...
import type { KnowledgeQuizHistoryListItem } from '@/lib/knowledge-quiz-history';
import { JOB_CATEGORIES } from '@/lib/leetcode-catalog';
...
function groupByDay(items: KnowledgeQuizHistoryListItem[]) {
  const groups = new Map<string, KnowledgeQuizHistoryListItem[]>();
...
function levelLabel(
  ...
  if (level === 'junior') return t.practiceQuizLevelJunior;
  if (level === 'senior') return t.practiceQuizLevelSenior;
  return t.practiceQuizLevelMid;
}

export default function KnowledgeQuizHistoryPanel({
  selectedId,
  onSelect,
  refreshKey = 0,
  onNew,
  ...
  const [items, setItems] = useState<KnowledgeQuizHistoryListItem[]>([]);
  ...
  useEffect(() => {
    ...
    fetch('/api/knowledge-quiz-history', {
      headers: { 'x-device-id': getDeviceId() },
      signal: controller.signal,
    })
      .then(async (res) => {
        const data = (await res.json()) as {
          items?: KnowledgeQuizHistoryListItem[];
          error?: string;
        };
  ...
  return (
    <div className="flex h-full min-h-0 flex-col text-[var(--foreground)]">
      <div className="flex items-start justify-between gap-2 px-4 pt-5 pb-3">
        <div>
          <h2 className="text-lg font-semibold">{t.quizHistory}</h2>
          <p className="text-xs text-[var(--shell-muted)] mt-0.5">{t.quizHistoryHint}</p>
        </div>
        {onNew && (
          <button
            type="button"
      ...
      <div className="flex-1 overflow-y-auto px-3 pb-4">
        ...
        {!loading && !error && items.length === 0 && (
          <div className="mx-1 mt-6 rounded-2xl border border-dashed border-[var(--shell-border)] bg-[var(--shell-card)]/60 px-4 py-8 text-center">
            <p className="text-sm text-[var(--shell-muted)]">{t.noQuizHistory}</p>
          </div>
        )}
        ...
        <div className="space-y-5">
          {groups.map(([day, dayItems]) => (
            <section key={day}>
              <h3 className="px-1 mb-2 text-xs font-medium text-[var(--shell-subtle)]">
                {day}
              </h3>
